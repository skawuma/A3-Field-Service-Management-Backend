...
public class DemoWorkOrderSeeder {
    ...
    public int seed(
        ...
        configureActiveSla(scanner, now.minus(220, ChronoUnit.MINUTES), 240);
        ...
        recordEvent(networkJack, WorkOrderEventType.TRAVEL_STARTED, "Technician started travel.",
                WorkOrderStatus.ASSIGNED.name(), WorkOrderStatus.EN_ROUTE.name(), DEMO_ACTOR_TECHNICIAN);
        configureActiveSla(networkJack, now.minus(150, ChronoUnit.MINUTES), 120);
        ...
        configureCompletedSla(monitor, monitor.getCompletedAt().minus(6, ChronoUnit.HOURS), 480);
        ...
        configureCompletedSla(teams, teams.getCompletedAt().minus(5, ChronoUnit.HOURS), 240);
    ...
    private WorkOrderEntity createWorkOrder(
        ...
        return workOrderRepository.save(WorkOrderEntity.builder()
                ...
                .priority(priority)
                .assignedAt(assignedTechId == null ? null : createdAt.plus(30, ChronoUnit.MINUTES))
                .createdAt(createdAt)
                .build());
    }

    private void configureActiveSla(WorkOrderEntity workOrder, Instant startedAt, int durationMinutes) {
        workOrder.setEnRouteAt(startedAt);
        if (workOrder.getStatus() == WorkOrderStatus.WORK_STARTED) {
            workOrder.setArrivedAt(startedAt.plus(30, ChronoUnit.MINUTES));
            workOrder.setWorkStartedAt(startedAt.plus(40, ChronoUnit.MINUTES));
        }
        workOrder.setSlaClockStartedAt(startedAt);
        workOrder.setSlaDurationMinutes(durationMinutes);
        workOrder.setSlaDueAt(startedAt.plus(durationMinutes, ChronoUnit.MINUTES));
        workOrder.setSlaBreached(Instant.now().isAfter(workOrder.getSlaDueAt()));
        if (Boolean.TRUE.equals(workOrder.getSlaBreached())) {
            workOrder.setBreachMinutes((int) Math.max(0,
                    ChronoUnit.MINUTES.between(workOrder.getSlaDueAt(), Instant.now())));
        }
        workOrderRepository.save(workOrder);
    }

    private void configureCompletedSla(WorkOrderEntity workOrder, Instant startedAt, int durationMinutes) {
        workOrder.setEnRouteAt(startedAt);
        workOrder.setArrivedAt(startedAt.plus(30, ChronoUnit.MINUTES));
        workOrder.setWorkStartedAt(startedAt.plus(45, ChronoUnit.MINUTES));
        workOrder.setSlaClockStartedAt(startedAt);
        workOrder.setSlaDurationMinutes(durationMinutes);
        workOrder.setSlaDueAt(startedAt.plus(durationMinutes, ChronoUnit.MINUTES));
        int actual = (int) ChronoUnit.MINUTES.between(startedAt, workOrder.getCompletedAt());
        int breach = (int) Math.max(0, ChronoUnit.MINUTES.between(workOrder.getSlaDueAt(), workOrder.getCompletedAt()));
        workOrder.setActualCompletionMinutes(actual);
        workOrder.setBreachMinutes(breach);
        workOrder.setSlaBreached(breach > 0);
        workOrderRepository.save(workOrder);
    }
